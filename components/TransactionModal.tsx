...
export default function TransactionModal({ isOpen, onClose, onSuccess }: TransactionModalProps) {
  ...
  useEffect(() => {
    ...
    if (category === "ALD Canister") {
      fetch("/api/items?category=ALD%20Precursor")
        .then(r => r.ok ? r.json() : [])
  ...
  const handleSave = async () => {
    ...
    if (category === "ALD Canister" && type === "입고") {
      ...
      if (inboundType === "충진" && !aldInitialGross) {
          ...
          <div>
            ...
            {category === "ALD Canister" && type === "입고" && inboundType === "충진" && barcodeId && (
              <div className="mt-3 space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-indigo-700 mb-1">
                    충진 후 Gross Weight (g) <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="number" step="0.001"
                    value={aldInitialGross}
                    onChange={e => setAldInitialGross(e.target.value)}
                    placeholder="업체 제공값 입력 (예: 498.000)"
                    className="w-full px-3 py-2.5 border border-indigo-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-400"
                  />
                  <p className="text-xs text-indigo-400 mt-1">
                    충진 후 저울에서 잰 전체 무게 (Canister + 물질)
                  </p>
                </div>
              </div>
            )}
